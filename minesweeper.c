...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  {
    b->board[i] = calloc(b->width, sizeof(int));
...
int countMines(board_t * b, int x, int y) {
  ...
  //WRITE ME!
  #if 0
  if (x > 0 && IS_MINE(b->board[y][x-1]))
  {
        nMine++;
  }
  if (x < b->width-1 && IS_MINE(b->board[y][x+1]))
  {
        nMine++;
  }
  if(y > 0){
    if (x > 0 && IS_MINE(b->board[y-1][x-1]))
    {
          nMine++;
    }
    ...
    if (x < b->width-1 && IS_MINE(b->board[y-1][x+1]))
    {
          nMine++;
    }
  }
